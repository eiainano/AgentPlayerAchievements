...
v0.1.6 完成。20 个 CLI 命令，5 个 Agent 双通道覆盖，**183 个成就**，**722 个测试全绿（29 文件）**。
...
- [x] 测试覆盖：722 tests, 29 files（Phase 1 + 全面审视修复 + init/verify/customize-api/server/doctor/uninstall/import）
...
| CLI 命令 | 3 个 | 20 个命令（含 uninstall/export/import/config/showcase/search/suggest/profile tools） |
