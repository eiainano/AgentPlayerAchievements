...
**AGPA 劣势:** 无 usage-based XP——Level 只在成就解锁时增长；无数据导出——用户数据安全无保障；Tiered 成就语法糖暂不需要（counter/threshold 已覆盖层级进展需求）。
...
**价值评估 (2026-06-05):** 🔴 核心基础设施。CC 是 AGPA 的主战场，JSONL 解析建立不依赖 Agent 意识的 ground truth 数据源。直接复活 3 个 token 成就（token_1m/token_titan/token_legend），并为未来所有精确统计提供可靠基础。
**参考:** cc-lens `lib/claude-reader.ts::parseSessionFile()` + history-viewer `src-tauri/src/providers/claude.rs`
**工作量:** ~2.5 天（含独立设计文档 `docs/superpowers/specs/2026-06-05-stop-jsonl-parsing-design.md`）
...
**价值评估 (2026-06-05):** 🔴 高价值。在 Token 统计基础上，按 model 分类展示各模型的 token 消耗分布，成就数据源头从 Agent 自报变为 JSONL ground truth。不包含成本估算（AGPA 不需要计费功能）。

**参考:** cc-lens `lib/pricing.ts` per-model 定价表
**工作量:** 1-2天（在 P0-1 基础上）
...
**价值评估 (2026-06-05):** 🔴 填补最明显的体验缺陷。成就解锁有间歇期（新手每天几个，老手几周一个），间歇期 Level 完全冻结。Usage XP 让 Level 持续增长，独立于成就解锁时间线。
...
**价值评估 (2026-06-05):** 🔴 数据基础设施。先采集 char_count/word_count 等数据，后续自然孵化"话痨"、"精简大师"类成就。这是成就系统的数据源投资，不是占坑废代码。
...
**价值评估 (2026-06-05):** 🔴 性能保障。当前 5228 事件无感知，但好的系统应该在瓶颈到达前把路修好。增量更新 + 预聚合让 Dashboard 热力图直接从日聚合读，不再逐事件扫描。日均 50-100 事件的增速下，几个月即可见收益。
**参考:** bashstats `src/db/writer.ts::recordDailyActivity()` + `daily_activity` 表
**工作量:** 2-3天

---

### 🟡 P1-4: 数据导出功能（⬆️ 从 P2 升级）

**差距:** 没有备份/导出。用户数月解锁的成就，`rm state.json` 就全没了。换机器 / 系统重装也无恢复途径。cc-lens 和 bashstats 提供了 export。这是数据安全基本保障，不是"高级功能"。
...
**价值评估 (2026-06-05):** 🔴 数据安全基本保障。`rm ~/.agent-achievements` 几个月的积累全归零，换机/重装无恢复途径。1-2天做完，独立功能不依赖其他模块。
...
**价值评估 (2026-06-05):** 🟢 远期代码质量投资。evaluator 11 种条件类型全在工作，115 测试全绿，没有要加的条件类型需求。一旦需要扩展新条件类型时再做，届时收益远超现在。保留在 P2，近期不碰。
...
#### P3-0b: 斐波那契 Nudge（⬇️ 从 P2-2 降级）

**降级原因 (2026-06-05):** guide 的 nudge 机制是在关键 session 节点提示尝试新功能。AGPA 已有 tip/hint 系统在成就解锁时做同样的事。区别仅是触发时机（"解锁时"→"session N 时"），收益太薄。AGPA 的 hint 系统已覆盖新手引导需求。
...
**删除原因 (2026-06-05):** 反刷分的场景是排行榜/多人竞争。AGPA 是纯本地、单人成就追踪器——用户"刷分"= 用更多 CC = 好事。cc-proficiency 需要反刷分因为它有社区 leaderboard。AGPA 不需要。不实施。

---

#### ❌ 每周目标系统（已删除）

**删除原因 (2026-06-05):** bashstats 的 weekly goals 绑定了一个完整的 XP multiplier + 排行榜 + 公开 profile 生态。脱离这个生态，"每周生成 3 个目标"就是一个孤立功能。AGPA 的 160 个成就本身就是持续的目标集合，再加每周目标是画蛇添足。不实施。
...
```
Phase 1 (1-2 周) — 数据基础
├─ P0-1: JSONL 解析 + P0-2: Token 统计 → ground truth 数据源，复活 3 个 token 成就
└─ P1-2: UserPromptSubmit Hook → char_count/word_count 数据基础设施

Phase 2 (1-2 周) — 体验提升
├─ P1-1: Usage-based XP → Level 持续增长，填补空窗期
└─ P1-4: 数据导出 / 导入 → 数据安全基本保障

Phase 3 (1-2 周) — 性能
└─ P1-3: 日聚合缓存表 → Dashboard 性能，未雨绸缪

Phase 4 (远期) — 代码质量
└─ P2-1: 规则引擎重构 → 渐进重构，等需要扩展新条件类型时再做
```

### P3 — 不实施，仅存档

以下建议经评估对 AGPA 意义不大，记录在案供未来参考：

| 编号 | 建议 | 不实施理由 |
|------|------|-----------|
| P3-0 | Tiered 成就语法糖 | counter/threshold 已覆盖层级进展，纯语法糖 |
| P3-0b | 斐波那契 Nudge | tip/hint 系统已覆盖新手引导 |
| P3-1 | 特征依赖图 | 10 个 category 分组已形成自然渐进路径 |
| P3-2 | Share Card | 偏离核心定位，Dashboard 已是最佳展示方式 |
| P3-3 | Session 回放 | 另一个产品品类，偏离成就追踪核心定位 |
| P3-4 | 反模式检测 | 正向激励系统，不适合做代码教练 |
| P3-5 | 叙事者角色 | 纯 cosmetic，零功能价值 |

已删除（不实施，不存档）：反刷分机制、每周目标系统
