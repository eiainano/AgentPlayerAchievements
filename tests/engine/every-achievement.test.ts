 ...
 *
 * For each of the 183 achievements, auto-generate minimal events that satisfy
...
function applyPredicate(expr: string, req: FilterReqs): void {
  ...
  // field < value (numeric) — set field just below threshold
  m = expr.match(/(\w+)\s+<\s*(\d+)/);
  if (m) { req.payload[m[1]!] = Math.max(0, Number(m[2]) - 1); return; }
}
