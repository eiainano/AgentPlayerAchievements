...
<p align="center">
  ...
  <a href="#"><img src="https://img.shields.io/badge/実績-217-blueviolet" alt="217 実績"></a>
  <a href="#"><img src="https://img.shields.io/badge/テスト-1207-green" alt="1207 テスト"></a>
  ...
  <a href="#"><img src="https://img.shields.io/badge/CLI-27コマンド-orange" alt="27 CLIコマンド"></a>
...
- **217個の実績**、11カテゴリ — 「Hello World」から「コンプリート」まで
...
| **MCPサーバー** | STDIOプロトコル（7ツール） | image.read, file.language_used, plan.mode_entered, user.message, automode.start |

両方のチャネルが同じ `~/.agent-achievements/` イベントログに書き込みます。エンジンは12種類の条件タイプで217個の実績を評価します。
...
- 🏆 **217個の実績**、11カテゴリ（オンボーディング、ツール熟練、マイルストーン、スキル、スタイル、ワークフロー、クリエイター、隠し、チャレンジ、コミュニティ、耐久）
...
| `agpa pack` | インストール済みのコミュニティ実績パックを一覧・表示 |
| `agpa banner` | CLIバナーのカラーテーマ切替 (Neon/Arcade/Gold) |
| `agpa history` | イベントログエントリを閲覧 |
| `agpa explain` | 実績のロック/解除理由を表示（条件分析） |
| `agpa watch` | リアルタイム実績進捗モニター |
| `agpa upgrade` | アップデートを確認してAGPAをアップグレード |
| `agpa completion` | シェル補完スクリプト生成 (bash/zsh/fish) |

> 完全なCLIリファレンス: `agpa --help`

## コミュニティパック

誰でも実績パックを作成して共有できます。YAMLファイルを`~/.agent-achievements/packs/`に置くだけでインストール完了です：

```bash
agpa pack list              # インストール済みパックを一覧表示
agpa pack info <id>         # パックの詳細を表示
```

パック形式、イベントカタログ、12の条件タイプについては[実績パック作成ガイド](docs/creating-achievements.md)を参照してください。
...
├── tool-registry.ts         # 中央ツール登録
├── cli/
│   ├── index.ts             # 統一CLIエントリ (27コマンド)
...
├── tools/                   # MCPツール定義 (7ツール)
├── utils/                   # 通知、検証、プロファイル、ピクセルアート、バッテリー等
├── verify/
│   └── auditor.ts           # 実績検証ロジック
├── config.ts                # グローバル設定
...
```bash
npm install          # 依存関係をインストール (5つのランタイム依存)
npm run build        # tsc --noEmit
npm test             # 1207テスト, 46ファイル
...
- **ランタイム** (5): `@modelcontextprotocol/sdk` · `yaml` · `zod` · `figlet` · 
- **開発**: `typescript` · `vitest` · `tsx`
- **オプション** (macOS): `terminal-notifier` — 解除時のシステム通知

> [!NOTE]
> **意図的に最小限。** 5つのランタイム依存関係、ランタイム時のネットワーク呼び出しゼロ。エンジンはJSONLストレージを使用する純粋関数 — 監査が容易で、壊れようがありません。
...
- **最小限の依存関係** — 5つのランタイム依存関係（`@modelcontextprotocol/sdk`、`yaml`、`zod`、`figlet`）— すべて広く監査済み。
...
脆弱性を報告するには、[SECURITY.md](SECURITY.md) をご覧ください。

## 👥 コントリビューション

コントリビューションを歓迎します！実績パック、ダッシュボードの改善、新しいツール統合、エンジンの修正など、あらゆるスキルレベルに参加方法があります。

- **[CONTRIBUTING.md](CONTRIBUTING.md)** — セットアップ、コーディング規約、PRプロセス、4つのコントリビューションパス
- **[実績パック作成ガイド](docs/creating-achievements.md)** — 実績定義の完全な作成ガイド
- **[`.github/ISSUE_TEMPLATE/`](.github/ISSUE_TEMPLATE/)** — IssueとPRのテンプレート
...
<p align="center">
  <sub>ゲーミフィケーションを愛する開発者のために。217個の実績、さらに増加中。</sub>
</p>
