...
- **218 个成就**，覆盖 11 个类别——从「Hello World」到「完美主义者」
...
两个通道都写入同一个 `~/.agent-achievements/` 事件日志。引擎用 12 种条件类型评估 218 个成就。
...
- 🏆 **218 个成就**，11 个类别（入门、工具精通、里程碑、技能、风格、工作流、创造者、隐藏、挑战、社区、耐力）
...
├── utils/                   # 通知、验证、日志、错误、配置
├── config.ts                # 全局配置
└── helpers.ts               # 共享工具函数

pixel-art-output/            # Logo + 成就像素画
achievement-definitions.yaml   # 218 个成就定义（权威数据源）
...
```bash
npm install          # 安装依赖 (3 个运行时依赖)
npm run build        # tsc --noEmit
npm test             # 1203 个测试, 45 个文件
...
> [!NOTE]
> **刻意保持精简。** 四个运行时依赖，零运行时网络调用。引擎是纯函数配合 JSONL 存储——易于审计，不可能出错。
...
- **依赖精简** — 4 个运行时依赖 (`@modelcontextprotocol/sdk`, `yaml`, `zod`, `figlet`) — 全部经过广泛审计。
...
<p align="center">
  <sub>为热爱游戏化的开发者而建。218 个成就，持续更新中。</sub>
</p>
