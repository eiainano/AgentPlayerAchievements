...
| **Kilo Code** | Kilo Org | ✅ stdio/HTTP | `AGENTS.md` > `CLAUDE.md` | ⭐⭐⭐⭐ | ❌ Hook 不可调 MCP |
| **Hermes Agent** | Nous Research | ✅ stdio/HTTP | `.hermes.md` > `AGENTS.md` > `CLAUDE.md` | ⭐⭐⭐⭐ | ✅ `on_session_end` |
| **OpenCode** | SST / Anomalyco | ✅ stdio/HTTP | `AGENTS.md` > `CLAUDE.md` | ⭐⭐⭐ | ⚠️ `session.idle` 近似 |
| **OpenClaw** | Peter Steinberger | ✅ stdio/SSE/HTTP | `AGENTS.md` > `SOUL.md` | ⭐⭐⭐⭐ | ⚠️ Hook handler 手写 |
...
配置文件：`~/.claude/claude_desktop_config.json` 或项目 `.mcp.json`
...
通过 SessionStart hook + memory 机制实现。在 `CLAUDE.md` 中写入 auto-track 指令，agent 于会话结束前自动调用 `achievement_track`。
