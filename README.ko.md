...
<p align="center">
  ...
  <a href="#"><img src="https://img.shields.io/badge/업적-213-blueviolet" alt="213 업적"></a>
  <a href="#"><img src="https://img.shields.io/badge/테스트-1176-green" alt="1176 테스트"></a>
  ...
  <a href="#"><img src="https://img.shields.io/badge/CLI-24_명령어-orange" alt="24 CLI 명령어"></a>
...
- **213개 업적**, 11개 카테고리 — "Hello World"부터 "컴플리셔니스트"까지
...
| **MCP 서버** | STDIO 프로토콜 (5개 도구) | image.read, file.language_used, plan.mode_entered, user.message, automode.start |

두 채널 모두 동일한 `~/.agent-achievements/` 이벤트 로그에 기록합니다. 엔진은 12가지 조건 유형으로 213개 업적을 평가합니다.
...
- 🏆 **213개 업적**, 11개 카테고리 (온보딩, 도구 숙련, 마일스톤, 스킬, 스타일, 워크플로우, 크리에이터, 히든, 챌린지, 커뮤니티, 인내)
...
├── tools/                   # MCP 도구 정의 (5개 도구)
├── utils/                   # 알림, 검증, 로그, 오류, 프로필
├── config.ts                # 전역 설정
└── helpers.ts               # 공유 유틸리티

pixel-art-output/            # 로고 + 업적 픽셀 아트
achievement-definitions.yaml   # 213개 업적 정의 (권위적 소스)
...
```bash
npm install          # 의존성 설치 (3개 런타임 의존성)
npm run build        # tsc --noEmit
npm test             # 1176개 테스트, 45개 파일
...
- **런타임** (4): `@modelcontextprotocol/sdk` · `yaml` · `zod` · `figlet`
- **개발**: `typescript` · `vitest` · `tsx`
- **선택 사항** (macOS): `terminal-notifier` — 해제 시 시스템 알림

> [!NOTE]
> **의도적으로 최소한으로.** 4개의 런타임 의존성, 런타임 시 네트워크 호출 없음. 엔진은 JSONL 저장소를 사용하는 순수 함수 — 감사하기 쉽고, 망가질 수 없습니다.
...
- **최소 의존성** — 4개의 런타임 의존성 (`@modelcontextprotocol/sdk`, `yaml`, `zod`, `figlet`) — 모두 광범위하게 감사됨.
...
<p align="center">
  <sub>게임화를 사랑하는 개발자를 위해 만들어졌습니다. 213개의 업적, 계속 증가 중.</sub>
</p>
