...
调研了 [subinium/claude-code-achievements](https://github.com/subinium/claude-code-achievements)（CCA），一个 29 成就的 Bash 插件。相比我们的 183 成就 TypeScript 引擎，CCA 在**用户可感知体验**上有几点做得更好：成就教育提示、聊天内查询、线索系统、跨平台通知、一键安装。以下整理哪些要采纳、哪些暂缓。
...
我们 183 个成就更侧重"记录行为 + 趣味奖励"，缺少从新手到大师的学习路径感。未来设计新成就时，应同时考虑两个维度：
1. **行为记录**：你做了什么
...
| **L9** | 6,400 | +1,700 | 8,100 | 全部 183 成就 ≈ Level 12 |
...
`N² × 100` 意味着 Level 5 需要 2,500 XP（50 个 Common 成就），Level 10 需要 10,000 XP（200 个 Common，但只有 183 个成就）。用户解锁全部 183 个成就的理论最大 XP 约为 `55×50 + 52×100 + 36×200 + 26×300 + 9×500 + 5×1000 = 2,750 + 5,200 + 7,200 + 7,800 + 4,500 + 5,000 = 32,450`。加上 task 约几百。理论最高 Level ≈ `floor(sqrt(33000/100))` = **Level 18**。
...
**AGPA 当前的状态**：我们有核心（183 成就），有反馈（音效+通知），但缺少**日常钩子**（daily quest、streak 保护、可变奖励）。Duolingo 告诉我们——光有成就列表不够，需要**每日理由**让用户打开工具。
...
12 项仔细评估后，**仅 Dashboard Streak 可视化值得借鉴**。理由：

| 功能 | 不值得做的原因 |
|------|--------------|
| Streak Freeze/Repair | 编码习惯与语言学习不同——一天不写代码很正常（周末、休假），不需要"惩罚"机制。Freeze 制造焦虑而非激励 |
| Daily Quests | AGPA 是被动追踪系统，不是主动任务平台。每日任务需要用户主动打卡，与当前"后台自动跑"的定位冲突 |
| XP Boost 道具 | XP 目前只是 Dashboard 展示数据，不参与任何实际机制。加 Boost 意味着需要限时状态管理，ROI 低 |
| Token 经济系统 | 需要完整的 earn→spend 循环 + 商店 UI，工程量大但对应的留存收益不明确（编码工具的 token 缺乏实际用途） |
| Weekly Challenge | 与 Daily Quests 同样的问题——AGPA 不应该变成任务管理器 |
| 周目标设定 | Dashboard 已经是被动展示面板，加目标设定需要用户交互 + 持久化 + 评估逻辑，偏离轻量定位 |
| 段位联赛 / 社交 | 全部需要多用户系统 |
| 角色进化 / 吉祥物 | 需要持续的美术资产投入 |
| 时段敏感奖励 | 编码时间不可控，强行给"晨间 bonus"毫无意义 |

**唯一值得做的一条**：Dashboard Streak 可视化。当前 `calcStreak()` 已经在计算连续天数，数据有了——只是没渲染。在 Hero section 加一张卡片，显示 🔥 连续天数和 📅 历史最高，改动面小、用户可感知、不改变任何系统行为。

**设计原则**：
- **不改引擎**：`calcStreak()` 已存在，API 已返回 `stats.streak`
- **不改事件流**：仍然靠 `session.start` 事件统计活跃天数
- **只加渲染**：Dashboard 新增一个 streak 卡片组件
- **不加机制**：不做 freeze/repair/wager——streak 是纯信息展示，不对用户施压

---

# 第四轮调研：GitHub 贡献热力图设计（2026-06-04）

> 分析 GitHub 用户主页的活动热力图（Contribution Graph），映射到 AGPA Dashboard。

---

## GitHub Contribution Graph 解剖

GitHub 用户主页的贡献图是全球最知名的编码活动可视化：
...
| 参数 | GitHub 值 | 说明 |
|------|----------|------|
| 网格维度 | 53 列 × 7 行 | 一年约 53 周，7 天/周 |
| 每格 = | 1 天 | 当天贡献数映射到颜色 |
| 时间窗口 | 过去 1 年 | 滚动窗口，每周左移一列 |
| 颜色层级 | 5 级 (0 → 4) | 0=无贡献(浅灰), 1-4=递增绿色 |
| 渲染方式 | SVG 或 CSS Grid | 每格是一个 `<rect>` 或 `<div>` |
| 纵轴标签 | Mon / Wed / Fri | 简洁，不标全部 7 天 |
| 横轴标签 | 月份名 (Jan–Dec) | 对齐到该月第一周 |
| 图例 | Less ░▒▓█ More | 5 级色阶说明 |
| Tooltip | 日期 + 贡献数 | hover 显示精确数值 |
| 数据源 | 服务端渲染 HTML partial | 异步请求返回预计算的矩阵 |

### 颜色方案

GitHub 使用 5 个绿色阶（从 `#ebedf0` 空 → `#216e39` 深绿），映射逻辑是**分位桶（quantile binning）**——不是固定阈值，而是按用户自己的历史数据分布动态分桶。这保证了每个人的热力图都有视觉区分度。

### 关键设计洞察

1. **空白格也是信息**：没写代码的日子不是缺失数据，它展示的是"间隔模式"——连续的空白块是休假、周末、荒废期。热力图的目标不是每格都绿，而是让模式可见。
2. **滚动窗口**：53 列覆盖一年，每周最左列滚动出去。这种"回头看一年"的尺度既能看到近期趋势，也能看到长期一致性。
3. **分位染色**：不用绝对值（如 ≥10 pushes = 深绿），而用分位数。老用户的浅绿天可能等于新用户的深绿天。热力图是"你跟自己比"。
4. **极简交互**：hover tooltip 是唯一交互。没有点击、没有过滤、没有动画。纯信息展示，但极其有效。
...
当前 `calcStreak()` 已经在收集活跃日期——我们只需要把这个日报数据扩展为矩阵，按天统计活动量。

```typescript
// 现在 calcStreak() 做的事：
// 收集所有 session.start 的日期去重 → sortedDays[]

// 热力图额外需要：
// 每个日期的活动强度（tool calls count / events count / session count）
```

### 核心设计决策

| 决策 | GitHub | AGPA 建议 | 理由 |
|------|--------|----------|------|
| 时间窗口 | 1 年 | **3 个月（~13 周）** | 编码工具快节奏，3 个月更能看到近期趋势 |
| 活动指标 | commits+PRs+reviews | **每日 session 数** | 数据已有、语义清晰。或可选每日 tool call 数 |
| 色阶级别 | 5 级 | **4 级**：0 / 1 / 2-3 / 4+ | 简化。session 数一般不会超过 10 |
| 渲染 | SVG | **CSS Grid** | 零依赖，与 Dashboard 技术栈一致 |
| 分位染色 | 是 | **固定阈值**：0 / 1 / 2-3 / 4+ | 分位染色对新用户（数据少）有问题 |
| 位置 | 独立区块 | **Streak 卡片下方** | streak 是"今天"，热力图是"过去"，自然延伸 |
...
13 列 × 7 行（3 个月 ≈ 13 周）
每格 13×13px，间距 3px
总宽度 ≈ 13 × (13+3) = 208px（适合 side panel 或紧凑展示）
grid-gap: 3px
```
...
```
0 sessions → var(--bg-surface)    灰色底（空）
1 session  → #c6e48b              浅绿（来过）
2-3 sessions → #7bc96f            中绿（活跃）
4+ sessions → #239a3b             深绿（高强度）
```

复用 Dashboard 现有的暗/亮主题变量，暗色模式下背景用 `--border` 色。

### 交互

- **hover tooltip**：显示日期 + 具体数字（"6月3日 · 3 sessions"）
- 纯 CSS tooltip（不需要 JS event listener），用 `title` 属性或 CSS `::after` + `data-*`
...
两者互补：streak 看今天，热力图看历史。放一起形成 Hero section 的"活动面板"。

---

## 待用户确认的关键问题

1. 活动指标：**每日 session 数** vs 每日 tool call 数 vs 每日事件总数？
2. 时间窗口：**3 个月** vs 6 个月 vs GitHub 同款的 1 年？
3. 位置：Streak 卡片下方合适吗？还是放在独立的"Activity" section？
