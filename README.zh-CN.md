...
- 🏆 **217 个成就**，覆盖 11 个类别 — 从「Hello World」到「完美主义者」
- 🔥 **GitHub 风格活动热力图** — 4 个月的编码活动一览无余
- 📸 **分享卡片** — 暗色/亮色主题，双语，可下载 PNG
- 🔊 **8-bit 音效与通知** — 按稀有度分级的复古音效 + 解锁时桌面推送通知
- 📂 **多配置文件** — 最多 4 个配置，随时切换（工作、个人、实验）
...
| `agpa profile` | 管理成就配置文件（创建、列表、切换、软件、删除） |
...
| `agpa sound` | 切换 8-bit 稀有度分级音效 (on, off) |
...
欢迎贡献！无论是成就包、Dashboard 改进、新工具接入，还是引擎修复——每个技能水平都有参与路径。

- **[CONTRIBUTING.md](CONTRIBUTING.md)** — 环境配置、编码规范、PR 流程与 4 条参与路径
- **[创建成就包](docs/creating-achievements.md)** — 编写成就定义的完整指南
- **[`.github/ISSUE_TEMPLATE/`](.github/ISSUE_TEMPLATE/)** — Issue 与 PR 模板
