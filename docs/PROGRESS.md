...
v0.1.8 完成。27 个 CLI 命令，5 个 Agent 双通道覆盖，**217 个成就**，**1204 个测试（1203 通过 / 46 文件）**。

### 当前 Triage（2026-06-15）

**已修复（今日）：**
- **非 CC 工具事件缺口** — 20 个成就对 Hermes/OpenClaw/KiloCode 不可达，全部修复至 0 真正不可达。OpenClaw/KiloCode 的 `tool.failure`/`error.occurred` 已 auto-emit；其余 ~12 个通过 CLAUDE.md 手动 track 指令覆盖——发射器存在，agent 严格执行即可解锁，属于手动 track 可靠性范畴（已暂缓）
- **`last_stand` 成就** — `engine.ts track()` 自动注入电池状态，CC + 非 CC 全通道修复
- **TypeScript 3 pre-existing 错误** — 全部修复，`tsc --noEmit` 零错误
...
**本次新增 (2026-06-15 非 CC 事件缺口修复):**
- **Cross-tool 审计**：发现 6/15 审查声明"0 不可达"仅对 CC 成立——Hermes/OpenClaw/KiloCode 用户有 20 个成就因 5 类事件无发射器而不可达
- **翻译层失败路由**：`normalizeOpenClawStdin()`/`normalizeKilocodeStdin()` — `success=false`/`error` 非空 → `PostToolUseFailure`，自动 emit `tool.failure` + `error.occurred`（原来无论成败都走 `PostToolUse`）
- **mapEvents error payload**：`PostToolUseFailure` case 现在将 `tool_input.error` 注入 payload
- **CLAUDE.md 指令扩展**：INSTRUCTION_BLOCK 新增 5 条手动 track 条目（`user.prompt`/`tool.failure`/`error.occurred`/`agent.spawn`/`context.compacted`）
- **Marker v1→v2 升级**：`injectInstructions()` 自动检测 v1 block 并替换为 v2，避免指令重复
- **引擎电池 enrichment**：`engine.ts track()` 对 `user.message` 自动注入 `on_battery`/`battery_pct`，4 行代码全通道修复 `last_stand`
- **TypeScript 修复**：auditor `layer: 'C'` 类型声明 + hook.ts `before_lines` unknown 断言
- **测试**：1161 → 1176（+15），hook.test.ts +10，init.test.ts +6
...
### 核心引擎
- [x] 12 种 Condition Type：Counter、Threshold（含 field 求和 + metric 表达式）、Streak、Sequence（含 consecutive 模式）、DistinctCount（含 values 白名单）、Event、SetCompletion、Mode、SequenceCount、PatternMatch、Ratio、TimeGap（Percentile 已移除）
- [x] YAML 解析器加载 217 个成就定义（含 12 个事件驱动型，异步发审计已全部修复）
- [x] PendingQueue 并发安全
- [x] MCP Tools：`achievement_track`、`achievement_poll`、`achievement_stats`、`achievement_showcase`、`achievement_config`、`achievement_suggest`
...
### 中优先级
- [x] **用户审阅 217 成就清单**：已逐条 review，修复 30+ 个不一致，删除 3 个重复，新增 1 个 + 语言系列 9 个 + 时间/测试 3 个 + 关键词/节日/电池系列 10 个
...
| CLI 命令 | 3 个 | 27 个命令（含 uninstall/export/import/config/showcase/search/suggest/profile/banner/activity/history/explain/pack） |
