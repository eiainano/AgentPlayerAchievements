...
describe('badge_image', () => {
  it('parses badge_image path from set definitions', () => {
    const yaml = `
definitions:
  - id: a1
    name: A1
...
sets:
  test_set:
    ...
    reward:
      type: badge
      value: Test Badge
    badge_image: "badges/test_set.png"
`;
    const result = parseYAML(yaml);
    expect(result.sets).toHaveLength(1);
    expect(result.sets[0]!.badge_image).toBe('badges/test_set.png');
  });

  it('sets badge_image to undefined when omitted', () => {
    const yaml = `
definitions:
  - id: a1
    name: A1
...
`;
    const result = parseYAML(yaml);
    expect(result.sets[0]!.badge_image).toBeUndefined();
  });
});
