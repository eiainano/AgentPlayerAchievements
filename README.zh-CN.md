...
- **183 个成就** — 覆盖 10 个分类，从 "Hello World" 到 "完美主义者"
...
两个通道写入同一份 `~/.agent-achievements/` 事件日志。引擎用 11 种条件类型评估 183 个成就。
...
- 🏆 **183 个成就** — 覆盖 10 个分类（入门、工具精通、里程碑、技能、风格、工作流、创造者、隐藏、挑战、社区）
...
04-成就定义清单.yaml          # 183 个成就定义（权威数据源）
...
> [!NOTE]
> **刻意极简。** 仅 3 个运行时依赖，零网络调用。引擎约 500 行纯函数 — 易于审计，难以出错。
...
<p align="center">
  <sub>为喜爱游戏化的开发者而建。183 个成就，还在增长中。</sub>
</p>
