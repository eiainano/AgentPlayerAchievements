...
```
Phase 1 (1-2 周) — P0 + P1 核心功能
├─ P0-1: JSONL 解析 → 精确 user message + token + session 时长
├─ P1-2: UserPromptSubmit Hook → 用户 prompt 字数追踪
├─ P1-4: 数据导出 / 导入 → 数据安全基本保障
└─ (可选) P1-1: Usage-based XP 双轨 → Level 持续增长

Phase 2 (2-3 周) — P1 收尾 + 性能
├─ P1-1: Usage-based XP（如 Phase 1 未做）
└─ P1-3: 日聚合缓存表 → Dashboard 性能
